...
async function loadGoogleMapsApi() {
  try {
    ...
    if (!apiKey) {
      console.error('Google Maps API key not found in keys.txt file. Create this file with: GOOGLE_MAPS_API_KEY = "your-api-key-here"');
...
function showApiKeyError() {
  ...
  if (errorDialog && errorMessage) {
    errorMessage.textContent = 'Google Maps API key not found. Location features will not work correctly.';
    errorDialog.showModal();
  } else {
    // Fall back to alert if error dialog isn't available
    alert('Google Maps API key not found. Location features will not work correctly.');
  }
}
